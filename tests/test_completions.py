...
class TestCompleteManagedBranches:
    def test_returns_managed_branch_names(self):
        ...
        with patch("wt_tool.cli.git.get_main_worktree_root", return_value=Path("/repo")), \
             patch("wt_tool.cli.git.list_worktrees", return_value=worktrees):
    ...
    def test_returns_empty_list_when_not_in_git_repo(self):
        with patch("wt_tool.cli.git.get_main_worktree_root", side_effect=SystemExit(1)):
            result = _complete_managed_branches()
        assert result == []

    def test_returns_empty_list_on_any_exception(self):
        with patch("wt_tool.cli.git.get_main_worktree_root", side_effect=RuntimeError("boom")):
            result = _complete_managed_branches()
        assert result == []
...
class TestCompleteBaseBranches:
    def test_returns_branch_list(self):
        with patch("wt_tool.cli.git.get_main_worktree_root", return_value=Path("/repo")), \
             patch("wt_tool.cli.git.list_branches", return_value=["main", "origin/feat"]):
    ...
    def test_returns_empty_list_on_any_exception(self):
        with patch("wt_tool.cli.git.get_main_worktree_root", side_effect=OSError("no git")):
            result = _complete_base_branches()
        assert result == []
